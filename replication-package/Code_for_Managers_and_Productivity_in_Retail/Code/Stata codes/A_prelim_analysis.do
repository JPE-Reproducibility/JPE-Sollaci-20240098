...
preserve
	...
	noisily {
		display in red " ============ The share of 'internal' manager changes is ============ "
		su share
	}
restore
...
noisily {
	display in red " ============ The number of managers per store is ============ "
	tab num_managers_store if aux == 1
	display in red " ============ ... and for stores that switched managers at least once: ============ "
...
noisily {
	display in red " ============ How many stores did each manager work in? ============ "
...
bys store_id (time): gen time_of_change = time if manager_id != manager_id[_n-1] & _n > 1 & _n < _N

// last change of manager
...
preserve
	...
	gen pct_mover  = 100*N[2]/(N[1]+N[2])
	gen pct_sales  = 100*(sales[2]-sales[1])/sales[1]
	gen pct_fte    = 100*(fte_count[2]-fte_count[1])/fte_count[1]
	gen pct_area   = 100*(store_area[2]-store_area[1])/store_area[1]
	gen pct_salary = 100*(manager_salary[2]-manager_salary[1])/manager_salary[1]
	
	noisily {
		display in red "============ Table B.1 ============"
		display in red "% Mover Stores " pct_mover[1]
		display in red "Sales  " pct_sales[1]
		display in red "FTE  " pct_fte[1]
		display in red "Store Size  " pct_area[1]
		display in red "Manager Salary  " pct_salary[1]
	}
	
	// save table
    keep if mover == 1  
    keep pct_mover pct_sales pct_fte pct_area pct_salary
    
    label var pct_mover  "% Mover Stores"
    label var pct_sales  "Sales (%)"
    label var pct_fte    "FTE (%)"
    label var pct_area   "Store Size (%)"
    label var pct_salary "Manager Salary (%)"

    export excel using "$res_dir/Tables/Table_B1_Company_A.xlsx", firstrow(variables) replace
restore
...
bys store_id: egen mover_i = max(mover)
...
tw line log_sales0 log_sales1 time, graphregion(color(white)) lcolor(blue red) lwidth(medthick medthick) lpattern(solid dash) ///
	ytitle("Log Sales", size(medium)) xtitle("") ///
	xlabel(1 "Apr-18" 4 "Jul-18" 7 "Oct-18" 10 "Jan-19" 13 "Apr-19" 16 "Jul-19" 19 "Oct-19" 22 "Jan-20" , angle(45) labsize(medium)) ///
...
tw line fte_count0 fte_count1 time, graphregion(color(white)) lcolor(blue red) lwidth(medthick medthick) lpattern(solid dash) ///
	ytitle("FTE Employment", size(medium)) xtitle("") ///
	xlabel(1 "Apr-18" 4 "Jul-18" 7 "Oct-18" 10 "Jan-19" 13 "Apr-19" 16 "Jul-19" 19 "Oct-19" 22 "Jan-20" , angle(45) labsize(medium)) ///
...
tw line store_area0 store_area1 time, graphregion(color(white)) lcolor(blue red) lwidth(medthick medthick) lpattern(solid dash) ///
	ytitle("Store Size (Thousand SqFt)", size(medium)) xtitle("") ///
	xlabel(1 "Apr-18" 4 "Jul-18" 7 "Oct-18" 10 "Jan-19" 13 "Apr-19" 16 "Jul-19" 19 "Oct-19" 22 "Jan-20" , angle(45) labsize(medium)) ///
...
tw line log_manager_salary0 log_manager_salary1 time, graphregion(color(white)) lcolor(blue red) lwidth(medthick medthick) lpattern(solid dash) ///
	ytitle("Log Manager Salary", size(medium)) xtitle("") ///
	xlabel(1 "Apr-18" 4 "Jul-18" 7 "Oct-18" 10 "Jan-19" 13 "Apr-19" 16 "Jul-19" 19 "Oct-19" 22 "Jan-20", angle(45) labsize(medium)) ///
	legend(off) ylabel(,labsize(medium)) name(mng_salary, replace)
	
grc1leg2 sales fte area mng_salary, graphregion(color(white))
graph export "$res_dir/Figures/Figure_A1_panel_A.png", width(1200) height(800) replace
...
gen log_prod = log(sales) - log(fte_count)

*** Table D.1 ***
// does productivity impact moves? 
noisily {
	display in red "============ Table D.1 ============"
	logit change_mng log_prod i.quarter#i.year
}	
	esttab using "$res_dir/Tables/Table_D1_Company_A.tex", replace ///
	b(3) se(3) label stats(N r2_p, fmt(0 3) labels("N" "Pseudo $R^2$"))
...
noisily {
	...
	clear
	svmat C, names(col)
	export delimited using "$res_dir/Tables/Table_C1.csv", replace
