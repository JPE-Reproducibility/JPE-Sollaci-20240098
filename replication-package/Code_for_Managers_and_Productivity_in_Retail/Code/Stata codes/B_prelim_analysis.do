...
preserve
	...
	gen pct_sales  = 100*(revenue_total[2]-revenue_total[1])/revenue_total[1]
	gen pct_fte    = 100*(fte_count[2]-fte_count[1])/fte_count[1]
	gen pct_area   = 100*(area_total[2]-area_total[1])/area_total[1]
	gen pct_mng_tenure = 100*(manager_tenure_start[2]-manager_tenure_start[1])/manager_tenure_start[1]
	...
	noisily {
		...
		display in red "Manager Tenure  " pct_mng_tenure[1]
	}
	
	// save table
    keep if mover == 1  
    keep pct_mover pct_sales pct_fte pct_area pct_mng_tenure
    
    label var pct_mover  "% Mover Stores"
    label var pct_sales  "Sales (%)"
    label var pct_fte    "FTE (%)"
    label var pct_area   "Store Size (%)"
    label var pct_mng_tenure "Manager Tenure (%)"

    export excel using "$res_dir/Tables/Table_B1_Company_B.xlsx", firstrow(variables) replace
restore
...
grc1leg2 revenue fte area mng_tenure, graphregion(color(white))
graph export "$res_dir/Figures/Figure_A1_panel_B.png", width(1200) height(800) replace
...
noisily {
	...
	esttab using "$res_dir/Tables/Table_D1_Company_B.tex", replace ///
