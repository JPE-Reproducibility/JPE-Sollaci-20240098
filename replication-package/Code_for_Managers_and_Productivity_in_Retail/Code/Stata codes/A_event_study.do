...
merge m:1 store_id manager_id time using "A_dataset_gfe", nogen
...
bys manager_id: egen manager_tenure_last = max(manager_tenure_comp)
...
* Is manager coming from out of state?
bys manager_id (time): gen mng_changed_state = (statefip != statefip[_n-1]) & _n > 1
bys manager_id store_id: egen aux = max(mng_changed_state)
replace mng_changed_state = aux
drop aux
...
forvalues n = 1/6 {
	...
	preserve
		...
		did_imputation `outcome' store_id time time_of_change, horizons(0(1)10) pretrend(6) autosample cluster(store_id) minn(5) // minn(5) required to run some of specifications below
		mat b = e(b)'
		mat v = vecdiag(e(V))'
...
*** Alternative design: date of change is event, but now there are pre-trends (Figure G.3)
did_imputation log_prod store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
...
graph export "$res_dir/Figures/Figure_G3_prod.png", width(1200) height(800) replace
	
did_imputation log_sales store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
event_plot, default_look ciplottype(rcap) graph_opt(xtitle("Months After Change in Mng.") ytitle("Log Sales") ///
	xlabel(-6(1)4) legend(off)) together
graph export "$res_dir/Figures/Figure_G3_sales.png", width(1200) height(800) replace

did_imputation ft_ratio store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
event_plot, default_look ciplottype(rcap) graph_opt(xtitle("Months After Change in Mng.") ytitle("Full/Part Time Ratio") ///
	xlabel(-6(1)4) legend(off)) together
graph export "$res_dir/Figures/Figure_G3_ft_ratio.png", width(1200) height(800) replace

did_imputation log_ft store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
event_plot, default_look ciplottype(rcap) graph_opt(xtitle("Months After Change in Mng.") ytitle("Log Full Time Employees") ///
	xlabel(-6(1)4) legend(off)) together
graph export "$res_dir/Figures/Figure_G3_ft.png", width(1200) height(800) replace

did_imputation log_pt store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
event_plot, default_look ciplottype(rcap) graph_opt(xtitle("Months After Change in Mng.") ytitle("Log Part Time Employees") ///
	xlabel(-6(1)4) legend(off)) together
graph export "$res_dir/Figures/Figure_G3_pt.png", width(1200) height(800) replace

did_imputation log_manager_salary store_id time time_of_change, horizons(0(1)4) pretrend(6) autosample cluster(store_id) minn(5)
event_plot, default_look ciplottype(rcap) graph_opt(xtitle("Months After Change in Mng.") ytitle("Log Manager Salary") ///
	xlabel(-6(1)4) legend(off)) together
graph export "$res_dir/Figures/Figure_G3_mng_salary.png", width(1200) height(800) replace
...
*** Managers and that leave vs managers that stay (Figure G.5)
...
preserve
	did_imputation `outcome' store_id time time_of_change if was_treated==0 | (was_treated==1 & top_bottom_mng==1), horizons(0(1)10) pretrend(6) autosample cluster(store_id) minn(5)
	mat b_tb = e(b)'
	mat v_tb = vecdiag(e(V))'

	did_imputation `outcome' store_id time time_of_change if was_treated==0 | (was_treated==1 & top_bottom_mng==0), horizons(0(1)10) pretrend(6) autosample cluster(store_id) minn(5)
	mat b_bt = e(b)'
...
did_imputation log_prod store_id time time_of_change if was_treated==0 | (was_treated==1 & top_bottom_mng==1), horizons(0(1)10) pretrend(6) autosample cluster(store_id) minn(5)
mat b_tb = e(b)'
	
did_imputation log_prod store_id time time_of_change if was_treated==0 | (was_treated==1 & top_bottom_mng==0), horizons(0(1)10) pretrend(6) autosample cluster(store_id) minn(5)
mat b_bt = e(b)'
...
preserve
	...
	// save table
    export excel using "$res_dir/Tables/Table_4_panel_A_Company_A.xlsx", firstrow(variables) replace
	putexcel set "$res_dir/Tables/Table_4_panel_A_Company_A.xlsx", modify
	putexcel B2:E4, nformat("0.000")
restore
...
preserve
	...
	// save table
    export excel using "$res_dir/Tables/Table_4_panel_B_Company_A.xlsx", firstrow(variables) replace
	putexcel set "$res_dir/Tables/Table_4_panel_B_Company_A.xlsx", modify
	putexcel B2:E4, nformat("0.000")
restore
...
* Manager Tenure
bys manager_id store_id: egen max_tenure_comp = max(manager_tenure_comp) // manager's tenure in company
bys manager_id store_id: egen min_tenure_comp = min(manager_tenure_comp)

bys store_id (time): gen aux = max_tenure_comp if _n == 1
bys store_id: egen mng_tenure_comp_before = mean(aux)
drop aux

bys store_id (time): gen aux = min_tenure_comp if _n == _N
bys store_id: egen mng_tenure_comp_after = mean(aux)
drop aux

gen mng_tenure_comp_chg = mng_tenure_comp_after - mng_tenure_comp_before
replace mng_tenure_comp_chg = . if mng_tenure_comp_before > 240  // retired managers


bys manager_id store_id: egen max_tenure_pos = max(manager_tenure_pos) // manager's tenure in position
bys manager_id store_id: egen min_tenure_pos = min(manager_tenure_pos)

bys store_id (time): gen aux = max_tenure_pos if _n == 1
bys store_id: egen mng_tenure_pos_before = mean(aux)
drop aux

bys store_id (time): gen aux = min_tenure_pos if _n == _N
bys store_id: egen mng_tenure_pos_after = mean(aux)
drop aux

gen mng_tenure_pos_chg = mng_tenure_pos_after - mng_tenure_pos_before
replace mng_tenure_pos_chg = . if mng_tenure_pos_before > 240  // retired managers
...
bys store_id: egen manager_out = max(mng_changed_state)
...
tw hist mng_tenure_pos_chg , lcolor(gray%25) fcolor(gray%25) bin(15) ///
|| kdensity mng_tenure_pos_chg, lcolor(black) lwidth(medthick) ///
...
graph export "$res_dir/Figures/Figure_A5_panel_A.png", width(1200) height(800) replace
...
graph export "$res_dir/Figures/Figure_7_panel_A.png", width(1200) height(800) replace ;
#delimit cr
...
graph export "$res_dir/Figures/Figure_7_panel_C.png", width(1200) height(800) replace ;
#delimit cr
...
preserve
	...
	graph export "$res_dir/Figures/Figure_D1_panel_A.png", width(1200) height(800) replace
restore
...
preserve
	...
	compress
	drop x y
	save "$res_dir/Tables/Table_2_Company_A", replace
...
gen pct_round = round(pct)
	
compress
drop x y
sort move_to start_in
save "$res_dir/Tables/Table_3_Company_A", replace
